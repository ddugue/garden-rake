...
RSpec.describe Garden::Fileset do
  ...
  it "should do simple path substitution" do
    subject << "a.txt"
    expect(subject.each.map { Garden::FileAwareString.create("prefix/%F").to_s })\
      .to eq(["prefix/test.txt", "prefix/a.txt"])
    expect(subject.map { Garden::FileAwareString.create("prefix/%F").to_s })\
      .to eq(["prefix/test.txt", "prefix/a.txt"])
  end
end
